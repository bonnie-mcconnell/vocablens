...
from tests.postgres_harness import seed_user
...
async def seed_experiment_registries(uow_factory) -> None:
    experiments = {
        "paywall_offer": [
...
async def set_profile(
    ...
    *,
    ...
    preferred_time_of_day: int = 18,
    frequency_limit: int = 2,
) -> None:
    async with uow_factory() as uow:
...
async def load_snapshot(session_factory, *, user_id: int, session_id: str) -> ScenarioSnapshot:
...
async def run_comeback_flow(harness) -> ScenarioSnapshot:
    uow_factory = UnitOfWorkFactory(harness.session_factory)
